...
describe('Refinement Tools (Integration)', () => {
  ...
  it('searchWebTool can search DuckDuckGo and return results', async () => {
    ...
    try {
      const result = await (tools.searchWebTool as any).execute({ query: 'React web framework' }, {} as any) as any;
  ...
  it('readWebpageTool can fetch and extract text from a real URL', async () => {
    ...
    const result = await (tools.readWebpageTool as any).execute({ url: 'https://example.com' }, {} as any) as any;
    ...
    assert.match(result.text, /documentation examples/i, 'Text should contain the updated example sentence');
  });
});
