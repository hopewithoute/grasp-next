...
describe('extractWebpageContent (Integration)', () => {
  it(
    'Layer 1 Success: extracts clean text from a standard unprotected website',
    async () => {
  ...
  it(
    'Layer 2 Fallback: successfully extracts from a heavily protected WAF site',
    async () => {
      // Jika kredensial tidak diset, batalkan (skip) tes ini agar tidak error konyol di CI
      if (!process.env.CLOUDFLARE_ACCOUNT_ID || !process.env.CLOUDFLARE_API_TOKEN) {
        console.warn(
          'Skipping Layer 2 integration test: CLOUDFLARE_ACCOUNT_ID / CLOUDFLARE_API_TOKEN not set.'
        );
        return;
      }
