...
6. Analyze the text for relationships between concepts, such as dependencies, prerequisites, or associations. If a relationship is implied, add it to the 'relationships' array.
7. relationClaims capture source-level statements that connect a concept to another named idea. Create a relationClaim when the source text uses phrases like "builds on", "requires", "depends on", "part of", "explains", or "connects to". Each relationClaim must have: "subjectText" (the concept being described), "predicate" (one of: "builds_on", "requires", "depends_on", "part_of", "explains", "related_to"), "objectText" (the related concept or idea), and "sourceRefs". Use "builds_on" for statements like "builds on foundational concepts of X".
8. When retrieval tools are available, call search-wiki-concepts before creating a new conceptKey for a topic that may already exist in the project knowledgebase. Use get-concept-context for likely matches so you can reuse existing concept keys, merge with existing concepts, and avoid duplicate relationships.
9. CRITICAL CONSTRAINTS: "relationshipType" MUST exactly be one of the following strings ONLY: ${allowedRelationships}. DO NOT invent or use any other relationship types!
10. Ensure the output strictly conforms to the JSON schema provided.`,
  },
] as const;
...
const INGESTION_USER_PROMPT =
  new PromptTemplate<IngestionPromptVars>(`{{draftSection}}{{retrievedSection}}## Chunk {{chunkNumber}} of {{totalChunks}} (sourceId: {{sourceId}})
