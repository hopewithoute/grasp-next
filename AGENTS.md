...
# Common Operations & Developer Workflow

A unified entry script `./run` is available at the root of the repository to manage common operations.

## Invoking the Script
* **Interactive Selector**: Run `./run` without arguments to launch a keyboard-selectable menu.
* **Direct Execution**: Run `./run <command>` to run a specific command directly.

## Available Commands

* **dev**: Starts all development servers (Next.js frontend & Python sidecars)
  ```bash
  ./run dev
  ```

* **build**: Builds Next.js production bundle
  ```bash
  ./run build
  ```

* **test**: Runs all tests (Vitest for frontend/TS workspaces & Pytest for Python services)
  ```bash
  ./run test
  ```

* **lint**: Runs JS/TS eslint and Python ruff check
  ```bash
  ./run lint
  ```

* **format**: Runs JS/TS prettier and Python ruff format
  ```bash
  ./run format
  ```

* **typecheck**: Runs TypeScript type checks workspace-wide (uses memory-safe tsgo)
  ```bash
  ./run typecheck
  ```

* **doctor**: Runs React Diagnostics
  ```bash
  ./run doctor
  ```
...
All commands run in the terminal are **automatically rewritten** by the Claude Code hook.
You do **NOT** need to prefix your commands with `rtk` in the documentation or when typing, as it is transparently handled under the hood.

Examples:
* `git status` (auto-rewritten to `rtk git status` under the hood)
* `vitest` (auto-rewritten to `rtk vitest` under the hood)
