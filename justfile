...
dev-lgs:
    cd services/evidence-kb && uv run uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload

# Start Embedding sidecar python service
dev-embedding:
    cd services/embedding-sidecar && uv run uvicorn main:app --host 127.0.0.1 --port 8766 --reload
